...
complete_years_12_19$Artist[complete_years_12_19$Artist == "og3ne"] <- "o'g3ne"
...
``` {r}
partial_years_09_12$Artist[partial_years_09_12$Artist == "ovi & paula seling"] <- "paula seling & ovi"
...
``` {r}
png("dance.png")
ggplot(data = scores_with_musical, aes(x = Place, y = danceability)) + geom_point() + theme_bw() + geom_smooth(method = 'lm') + xlab("Place in Eurovision Final") + ylab("Danceability")
dev.off()


ggplot(data = scores_with_musical, aes(x = Place, y = tempo)) + geom_point() + theme_bw() + geom_smooth(method = 'lm')
```
