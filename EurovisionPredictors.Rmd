...
dyadseighties<- subset(dyadseighties, eighties.avg > 0.15)
dyadsnineties <- subset(dyadsnineties, nineties.avg > 0.15)
dyadstwothousands <- subset(dyadstwothousands, twothousands.avg > 0.15)
dyadstwentytens <- subset(dyadstwentytens, twentytens.avg > 0.15)
...
par(bg=NA)
plot(net, edge.arrow.size=.2, edge.color="white", vertex.color="darkblue", vertex.frame.color="#ffffff", vertex.label=V(net)$nodes, vertex.label.color="white", edge.width=E(net)$weight, size = 300, vertex.label.cex = 0.5)
dev.off()
```
...
fullscores <- merge(fullscores, countries, by.x = "To", by.y = "StateNme")
fullscores <- merge(fullscores, countries, by.x = "From", by.y = "StateNme", suffixes = c(".To", ".From"))
...
``` {r}
fullscores$dist <- fullscores$dist / 1000
...
storethis <- lm(data = fullscores, correctnormalization ~  To +  as.numeric(Draw) +  english + To*From + Year +  dist + neighbor + seventies * To + eighties * To + nineties * To + twothousands * To + twentytens * To + ally + mid)
summary(storethis)
...
storethis$coefficients[names(storethis$coefficients) == "mid"]*2
```

``` {r}
no_decades <- lm(data = fullscores, correctnormalization ~  To +  as.numeric(Draw) +  english + To*From + Year +  dist + neighbor)

no_neigh <- lm(data = fullscores, correctnormalization ~  To +  From + To*From + Year)
summary(no_neigh)

no_interact <- lm(data = fullscores, correctnormalization ~  To +  as.numeric(Draw) +  english + Year +  dist + neighbor)
summary(no_interact)
...
``` {r}
place <- lm(data = fullscores, Place ~  To +  as.numeric(Draw) +  english + To*From + Year +  dist + neighbor + seventies * To + eighties * To + nineties * To + twothousands * To + twentytens * To + ally)
